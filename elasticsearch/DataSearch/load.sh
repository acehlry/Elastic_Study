...
# load index template
if curl -f -XPUT "$url/_index_template/web-logs" -k -u $username:$password -H 'Content-Type: application/json' -d "@web-logs-template.json"
...
if curl -f -XPUT "$url/_ingest/pipeline/web-logs" -k -u $username:$password -H 'Content-Type: application/json' -d "@web-logs-pipeline.json"
